...
// ========== ERROR HANDLING ==========
if (exception != null) {
    System.out.println("[ERROR] [JSP] Exception occurred: " + exception.getMessage());
...
System.out.println("[DEBUG] ========= JSP STARTED RENDERING =========");
System.out.println("[DEBUG] JSP Path: " + request.getRequestURI());
System.out.println("[DEBUG] Request Attributes: " + Collections.list(request.getAttributeNames()));
System.out.println("[DEBUG] Session Attributes: " + Collections.list(session.getAttributeNames()));
%>
...
if (user != null) {
    ...
    System.out.println("[DEBUG] User found - ID: " + userId + ", Name: " + userName);
}

if (student != null) {
    semester = student.getSemester();
    System.out.println("[DEBUG] Student found - Semester: " + semester);
}
%>
   ...
   <% if ("student".equals(userRole)) { %>
    <div class="main-content">
                ...
                <div class="proposal-details">
                    ...
                    <div class="detail-row">
                        ...
                        <span class="detail-value <%= existingProposal.getStatus().toLowerCase() %>">
                            <% 
                            String status = existingProposal.getStatus();
                            out.print(Character.toUpperCase(status.charAt(0)) + status.substring(1).toLowerCase());
                            %>
                        </span>
                    </div>
                    <% if ("Accepted".equals(existingProposal.getStatus())) { %>
                        <div class="detail-row">
                            <span class="detail-label">Supervisor</span>
                            <span class="detail-value">
                                <i class="fas fa-user-tie"></i>
                                <%= request.getAttribute("supervisorName") != null ? 
                                    request.getAttribute("supervisorName") : "Not assigned" %>
                            </span>
                        </div>
                    <% } %>
                    <div class="detail-row">
        ...
        <% } %>
          ...
          <% if (existingProposal == null || !"Accepted".equals(existingProposal.getStatus())) { %>
            <form action="ProposalServlet" method="post" enctype="multipart/form-data">
                ...
                <div class="form-row">
                  ...
                  <div class="form-group half-width">
                    ...
                    <input type="text" name="title" 
                           value="<%= existingProposal != null ? existingProposal.getTitle() : "" %>" 
                           <%= "Accepted".equals(existingProposal != null ? existingProposal.getStatus() : "") ? "readonly" : "required" %>>
                  </div>
                </div>

                <div class="form-group full-width">
                    <label>Scope:</label>
                    <select name="scope" <%= "Accepted".equals(existingProposal != null ? existingProposal.getStatus() : "") ? "disabled" : "required" %>>
                ...
                <div class="form-group full-width">
                  ...
                  <div class="upload-section">
                    ...
                    <input type="file" id="upload" name="proposalFile" accept=".pdf" 
                           <%= existingProposal == null ? "required" : "" %>
                           <%= "Accepted".equals(existingProposal != null ? existingProposal.getStatus() : "") ? "disabled" : "" %>>
                ...
                <div class="buttons">
                    ...
                    <% if (!"Accepted".equals(existingProposal != null ? existingProposal.getStatus() : "")) { %>
                    ...
                    <% } else { %>
                        <div class="alert info" style="margin-top: 1rem;">
                            <i class="fas fa-info-circle"></i> Your proposal has been accepted and can no longer be modified.
                        </div>
                    <% } %>
                </div>
            </form>
...
function confirmDelete() {
    Swal.fire({
        ...
        text: "Are you sure you want to delete your proposal? This action cannot be undone.",
        icon: 'warning',
...
<style>
    ...
    /* Optional: Add background colors for more emphasis */
    ...
    .detail-value.status-badge.accepted {
        background-color: #f0fff4;
    }
    
    .detail-value.status-badge.pending {
        background-color: #fffaf0;
    }
    
    .detail-value.status-badge.rejected {
        background-color: #fff5f5;
    }
</style>

<style>
    /* Supervisor icon styling */
    .detail-value .fa-user-tie {
        color: #4b2e83; /* Purple color to match your theme */
        margin-right: 0.5rem;
    }
    
    /* Additional styling for supervisor row */
    .detail-row.supervisor-row {
        background-color: #f9f7fd; /* Light purple background */
        border-radius: 0.25rem;
        padding: 0.75rem;
    }
</style>
