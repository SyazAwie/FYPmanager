    ...
    protected void doGet(HttpServletRequest request, HttpServletResponse response)
        ...
        try {
            ...
            try (Connection conn = DatabaseConnection.getConnection()) {
                ...
                // 1. Load existing F7 data if available
                Map<String, String> formData = formsDB.getFormF7Data(userId);
                for (Map.Entry<String, String> entry : formData.entrySet()) {
                    request.setAttribute(entry.getKey(), entry.getValue());
                }

                // 2. Fetch supervisor and examiner scores
                Double supervisorScore = formsDB.getF7ScoreByRole(userId, "supervisor");
                Double examinerScore = formsDB.getF7ScoreByRole(userId, "examiner");
                ...
                // 4. Calculate final combined score: 10% + 5%
            ...
            RequestDispatcher dispatcher = request.getRequestDispatcher("f7.jsp");
            dispatcher.forward(request, response);

        } catch (SQLException | NumberFormatException e) {
            e.printStackTrace();
            response.sendRedirect("error.jsp?msg=F7FormLoadError");
        }
    }
}
